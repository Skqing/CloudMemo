...
;(function($, window, document){
    $.fn.waterfall = function(options){
        var // 配置信息
            ...
            minColsIndex = 0,	// 最高列的下标

            jsonCache = [],		// 服务器返回的JSON缓存数据

            wf_col_top = 0,		// $wf_col 的相对视图的位置高度
            wf_item_top = 0,	// 瀑布流块的top, left值
            wf_item_left = 0,

        // 一些jQ对象
            $wf_box, $wf_inner,
            $wf_col, $wf_col_temp, $wf_col_items,
            $wf_result, $backTop,
        ...
        function dealData(){
            ...
            loadImg(jsonCache, function(){
                while(colNum-- > 0 && (data = jsonCache.pop())){

                    minColsIndex = getColsIndex(colsHeight)[0];

                    wf_item_left = minColsIndex * (opts.colWidth + opts.marginLeft);
                    wf_item_top = colsHeight[minColsIndex] ?
                        (colsHeight[minColsIndex] + opts.marginTop) : colsHeight[minColsIndex];

                    htmlStr = createHtml(data);

                    $wf_item = $('<div>').addClass(opts.itemClass).html(htmlStr).appendTo($wf_col);
                    $wf_img = $wf_item.find('.'+opts.imgClass);
                    $wf_img.height($wf_img.width() / data.width * data.height);

                    if(opts.isAnimation){
                        $wf_item.css({width:opts.colWidth, opacity:0, left: 0, top: wf_col_height})
                            .stop(true)
                            .animate({
                                opacity: 1,
                                left: wf_item_left,
                        ...
                        $wf_item.css({width:opts.colWidth, left: wf_item_left, top: wf_item_top});
                    }
        ...
        return this.each(function(){
            if($(this).data('__wf__')) return true;

            $wf_box = $(this).addClass('waterfall').data('__wf__', true);
